...
describe('specificActionERC20TransferBatchBuilder', () => {
  ...
  it('should create a caveat with a valid specific action ERC20 transfer batch', () => {
    ...
    expect(caveat.terms.slice(endOfTokenAddress, endOfRecipient)).toStrictEqual(testRecipient.slice(2));
    ...
    expect(caveat.terms.slice(endOfRecipient + 64, endOfFirstTarget)).toStrictEqual(testFirstTarget.slice(2));
    expect(caveat.terms.slice(endOfFirstTarget)).toStrictEqual(testFirstCalldata.slice(2)); // 0x removed from the appended data
  });
